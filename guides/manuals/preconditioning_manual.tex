...
\begin{figure}[h]
  ...
  \includegraphics[height=1.95in]{photos/ui_2026-08-27.png} }  \\
   \caption{Screenshot of ``Buttons'' page in WiCAN http configuration.}
\end{figure}

The first option, ``Preconditioning Mode'', selects how long preconditioning continues once triggered:
\begin{enumerate}
...
The second option allows you a choice of button to map preconditioning onto. By default, 6 options are displayed:
\begin{enumerate}
  \item Disabled
...
\end{enumerate}
The ``Setup'' button is only present on EV6 climate bars. More options are available with the customized WiCAN in man-in-the-middle mode.
...
The third option, ``Preconditioning Press Type'', lets you choose whether the activation button triggers on a short press or a long press. With ``Short Press'', preconditioning triggers when you press and release the button within one second. With ``Long Press (hold 1s)'', you need to hold the button for at least one second; this can help avoid accidental triggering while driving.

The fourth option, ``CAN Connection Mode'', is only shown when the WiCAN is connected to two CAN busses (the customized WiCAN in man-in-the-middle mode). With ``MITM (forward between buses)'', frames are forwarded between the two busses so the head unit and the rest of the car keep working normally, and preconditioning requests can be intercepted. With ``Parallel (no forwarding)'', no frames are forwarded. See the \href{https://github.com/tylerharvey/Ioniq5_CAN/blob/main/guides/harnesses/head_unit_MITM/MITM_harness_modes.pdf}{harness modes guide} for more detail on MITM vs.\ parallel.

The first tab, ``Status'', shown in Figure \ref{subfig:ui:status}, shows the current high-voltage battery temperature (minimum and maximum across the battery), which is a convenient way to check whether preconditioning conditions are met (see below).

\begin{figure}[h]
  \subfloat[]{\label{subfig:ui:status}
  \includegraphics[height=1.95in]{photos/ui_status_2026-08-27.png} }  \\
   \caption{Screenshot of the ``Status'' page in WiCAN http configuration, including the high-voltage battery temperature readout.}
\end{figure}
